...
describe("AttachmentSection Component", () => {
    ...
    it("should render active attachments", () => {
        render(_jsx(AttachmentSection, { ticketId: 1, requesterId: 1, attachments: mockAttachments, onAttachmentChanged: () => { } }));
        expect(screen.getByText("test.png")).toBeInTheDocument();
    });
    it("should handle soft removal", async () => {
        const mockOnChanged = vi.fn();
        window.prompt.mockReturnValueOnce("Wrong file");
        globalThis.fetch.mockResolvedValueOnce({
            ok: true,
            json: async () => ({ message: "Removed" })
        });
        render(_jsx(AttachmentSection, { ticketId: 1, requesterId: 1, attachments: mockAttachments, onAttachmentChanged: mockOnChanged }));
    ...
    it("should reject large files locally", async () => {
        render(_jsx(AttachmentSection, { ticketId: 1, requesterId: 1, attachments: [], onAttachmentChanged: () => { } }));
